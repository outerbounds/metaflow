...
def _compute_executor_max_workers():
    # For processes, let's start conservative. Let's restrict to 4-24 always. Can be configurable one day.
    min_processes = 4
    max_processes = 16
    effective_cpu_limit = (
        _determine_effective_cpu_limit()
...
class AzureStorageExecutor(object):
    ...
    def __init__(self, use_processes=False):
        ...
        print("WORKERS: %d %d" % (processpool_max_workers, threadpool_max_workers))
        if use_processes:
