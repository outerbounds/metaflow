...
class TestAzureStorage(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        storage = DATASTORES["azure"](self.datastore_root)
    ...
    def test_size_file(self):
        ...
        self.assertEqual(self.storage.size_file("does_not_exist"), None)
    ...
    def test_is_file_many_check_client_cache(self):
        self.storage = DATASTORES["azure"](TestAzureStorage.generate_datastore_root())

        # we reset the client cache, because we want to strictly check cache size later
        from metaflow.plugins.azure.blob_service_client_factory import _ClientCache

        _ClientCache._cache = dict()
        ...
        if self.storage._use_processes:
            self.assertEqual(len(_ClientCache._cache), 0)
        else:
            self.assertGreaterEqual(len(_ClientCache._cache), 1)
